...
import banner from './category-banner-1.webp';
import classes from './FV.module.css';
...
const FV = () => {
  ...
  useEffect(() => {
  sub_categories.map(data=>(data.category_name === 'fruits & vegetables' ? setId(data._id):''))
   
 },[])
  useEffect(() => {
    if (Array.isArray(sub_categories)) {
      const matchedCategory = sub_categories.find(
        (data) => data.category_name === "fruits & vegetables"
          ...
          <Col lg={4} className="p-0">
            ...
            <ul className={classes.sub_category}>
            {
                ...
                sub_categories.map(data => (
                  data.category_name === 'fruits & vegetables' ?
            ...
            <button className={classes.shop} onClick={() => {navigate(`/products/category/${id}`)}}>shop all fruits & vegetables <FaArrowRightLong/></button>
          </Col>
